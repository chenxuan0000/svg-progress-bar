...
vueProgress.prototype = {
	_generate: function() {
		...
		this._radiusAdjusted =
			this._radius -
			(this._strokeWidthArray
				? Math.max(this._strokeWidthArray[0], this._strokeWidthArray[1]) / 2
				: this._strokeWidth / 2)
		this._generateSvg()
			._generateText()
			._generateWrapper()
	...
	_generateSvg: function() {
		...
		this._generatePath(
			...
			this._colors[0],
			this._maxValClass
		)._generatePath(0, true, this._colors[1], this._valClass)
		if (this._type === 'circle') {
			...
			this._svg.setAttribute(
				'height',
				this._strokeWidthArray
					? Math.max(this._strokeWidthArray[0], this._strokeWidthArray[1])
					: this._svgHeight
			)
		}
	...
	_generatePath: function(percentage, open, color, pathClass) {
		...
		if (this._type === 'circle') {
			...
			this._setCss(path, {
				...
				'stroke-width': this._strokeWidthArray
					? open
						? this._strokeWidthArray[1]
						: this._strokeWidthArray[0]
					: this._strokeWidth
			})
			...
			if (this._strokeWidthArray) {
				let delated =
					(this._strokeWidthArray[1] - this._strokeWidthArray[0]) / 2
				if (delated > 0) {
		...
		;(function animate(lastFrame) {
			...
			if (self._valAddCalBack.length > 0) {
				self._valAddCalBack.forEach(item => {
					if (
						item.value - oldPercentage > 0 &&
						item.value - oldPercentage <= judgeRate
					) {
						item.func()
					}
				})
